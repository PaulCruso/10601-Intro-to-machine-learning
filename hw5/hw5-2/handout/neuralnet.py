...
parser.add_argument('num_ep*och', type=int,
...
class SoftMaxCrossEntropy:
    ...
    def _cross_entropy(self, y: int, y_hat: np.ndarray) -> float:
        ...
        """
        equal to sum(y * log(y_hat)), because other label is 0
        """
        true_label = 1

        false_label = 0

        return true_label * -np.log(y_hat[y])
    ...
    def backward(self, y: int, y_hat: np.ndarray) -> np.ndarray:
        ...
        """
        According to problem 2 (c), we only deduct the true label refers to y, since other label is 0
        """

        gradient[y] = gradient[y] - true_label
...
class Linear:
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        self.dw = np.dot(dz.reshape(-1, 1), self.x.T)

        w_hat = self.w[:, 1:]

        if dz.ndim == 1:
            dz = dz.reshape(-1, 1)
        dx = np.dot(w_hat.T, dz)
...
class NN:
    def __init__(self, input_size: int, hidden_size: int, output_size: int,
        ...
        self.layers = [Linear(input_size, hidden_size, weight_init_fn, learning_rate),
                       Linear(hidden_size, output_size, weight_init_fn, learning_rate)]
    ...
    def forward(self, x: np.ndarray, y: int) -> Tuple[np.ndarray, float]:
        ...
        hidden_layer = self.linear1.forward(x)
        activation = self.sigmoid.forward(hidden_layer)
        output_layer = self.linear2.forward(activation)

        y_hat, loss = softmax.forward(output_layer, y)
    ...
    def backward(self, y: int, y_hat: np.ndarray) -> None:
        ...
        softmax = SoftMaxCrossEntropy()
        output_gradient = softmax.backward(y, y_hat)
        hidden_gradient = self.linear2.backward(output_gradient)
        activation_gradient = self.sigmoid.backward(hidden_gradient)
        input_gradient = self.linear1.backward(activation_gradient)
    ...
    def compute_loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        for index in range(len(X)):
            y_hat, loss = self.forward(X[index], y[index])
            total_loss += loss
    ...
    def train(self, X_tr: np.ndarray, y_tr: np.ndarray,
        ...
        for epoch in range(n_epochs):
            ...
            for i in range(len(X_tr_shuffle)):
                x = X_tr_shuffle[i]
                Y = y_tr_shuffle[i]
            ...
            tr_loss = self.compute_loss(X_tr_shuffle, y_tr_shuffle)[0]
            test_loss = self.compute_loss(X_test, y_test)[0]
    ...
    def test(self, X: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, float]:
        ...
        for count, x in enumerate(X):
            y_hat, loss = self.forward(x, y[count])
        ...
        error_rate = count / len(predictions)
